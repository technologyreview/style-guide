...
var Twig = function (Twig) {
	...
	Twig.token.findStart = function (template) {
		var output = {
			...
			i, token_template, first_key_position;
...
var Twig = function (Twig) {
	...
	(function () {
		...
		Twig.lib.formatDate = function (date, format) {
			if (typeof format !== "string" || /^\s*$/.test(format)) return date + "";
		...
		parse: function (token, context, continue_chain) {
			var result = Twig.expression.parse.apply(this, [token.expression, context]),
				...
				loop = function (key, value) {
					...
					if (conditional === undefined || Twig.expression.parse.apply(that, [conditional, inner_context])) {
...
var Twig = function (Twig) {
	"use strict";
	Twig.expression = {};
	Twig.expression.reservedWords = ["true", "false", "null", "_context"];
	...
	Twig.expression.definitions = [{
		...
		compile: function (token, stack, output) {
			...
			if (value === ":") {
				if (stack[stack.length - 1] && stack[stack.length - 1].value === "?") {} else {
					...
					if (key_token.type === Twig.expression.type.string || key_token.type === Twig.expression.type.variable || key_token.type === Twig.expression.type.number) {
		...
		compile: function (token, stack, output) {
			...
			if (token === undefined || token.type !== Twig.expression.type._function && token.type !== Twig.expression.type.filter && token.type !== Twig.expression.type.test && token.type !== Twig.expression.type.key.brackets && token.type !== Twig.expression.type.key.period) {
